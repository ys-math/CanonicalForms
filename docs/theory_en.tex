...
canonical form for a binary relation $\sim$ on a set $X$, and study them
mainly when $\sim$ is an equivalence relation.
...
also implies $x \sim y$.
A canonical form is an invariant $C \colon X \to X$ satisfying $x \sim
C(x)$ for all $x \in X$; its image $C(X)$ is called the skeleton of $\sim$,
and, when $\sim$ is an equivalence relation, also the complete system of
...
through the surjective complete invariant $I' \colon X \to I(X)$. For a
section $s'$ of $I'$, we call $s' \circ I'$ the canonical form of $\sim$
determined by $I$ and $s'$, and show that it is a canonical form of $\sim$.
Assuming the axiom of choice, we show that $I'$ has a section, so that such a
canonical form exists for every complete invariant $I$.
Propositions that assume the axiom of choice are marked
with~$^{\textrm{AC}}$.
\end{abstract}

\begin{definition}
% Lean: CanonicalForms.IsInvariant, CanonicalForms.IsCompleteInvariant (CanonicalForms/Basic.lean)
Let $X, Y$ be sets. Let $f \colon X \to Y$ be a map and let $\sim$ be a
binary relation on $X$. We say that $f$ is an \textbf{invariant} of $\sim$
...
\begin{example}
% Lean: CanonicalForms.quotientMap_isCompleteInvariant (CanonicalForms/Basic.lean)
...
\begin{definition}
% Lean: CanonicalForms.IsCanonicalForm, CanonicalForms.skeleton (CanonicalForms/CanonicalForm.lean)
Let $X$ be a set. Let $C \colon X \to X$ be an invariant of a binary
relation $\sim$ on $X$. We say that $C$ is a \textbf{canonical form} of $\sim$
if the following condition holds.
\begin{center}
  ${^{\forall}x} \in X,\ x \sim C(x)$
\end{center}
We call the image $C(X)$ of $X$ under $C$ the \textbf{skeleton} of $\sim$.
When $\sim$ is an equivalence relation, we also call $C(X)$ the
\textbf{complete system of representatives} of $\sim$.
\hfill $\square$
\end{definition}

\begin{proposition}
% Lean: CanonicalForms.canonicalForm_idempotent (CanonicalForms/CanonicalForm.lean)
Let $X$ be a set. Let $C \colon X \to X$ be a canonical form of a binary
relation $\sim$ on $X$. Then $C = C \circ C$.
\hfill $\square$
\end{proposition}
...
\begin{definition}
% Lean: CanonicalForms.IsRetractionSectionPair (CanonicalForms/Basic.lean)
...
\begin{lemma}
% Lean: CanonicalForms.canonicalForm_iff_section (directions: canonicalForm_to_section, section_to_canonicalForm) (CanonicalForms/CanonicalForm.lean)
...
\begin{proposition}
% Lean: CanonicalForms.canonicalForm_iff_section_of_completeInvariant (CanonicalForms/CanonicalForm.lean)
...
\begin{proposition}
% Lean: CanonicalForms.corestriction, CanonicalForms.corestriction_existsUnique (via val_comp_corestriction, corestriction_unique), CanonicalForms.corestriction_surjective_completeInvariant (CanonicalForms/Basic.lean)
...
\begin{proof}
Define $I' \colon X \to I(X)$ by $I'(x) = I(x)$ for all $x \in X$. Then
$i(I'(x)) = I(x)$ for all $x \in X$, so $I = i \circ I'$; this shows that such
a map exists. Since $I'(X) = I(X)$, $I'$ is surjective. For any $x, x' \in X$,
$x \sim x' \iff I(x) = I(x') \iff I'(x) = I'(x')$,
so $I'$ is a complete invariant of $\sim$. If $I'' \colon X \to I(X)$ also
satisfies $I = i \circ I''$, then $i(I'(x)) = I(x) = i(I''(x))$ for all
$x \in X$, and since $i$ is injective, $I'(x) = I''(x)$. Therefore $I'' = I'$,
so $I'$ is unique.
\end{proof}

\begin{definition}
% Lean: CanonicalForms.canonicalFormOfCompleteInvariant (CanonicalForms/Basic.lean)
Let $X, Y$ be sets and let $I \colon X \to Y$ be a complete invariant of an
equivalence relation $\sim$ on $X$. Let $I' \colon X \to I(X)$ be the unique
map satisfying $I = i \circ I'$ with respect to the inclusion
$i \colon I(X) \to Y$, and let $s' \colon I(X) \to X$ be a section of $I'$.
We call the map $s' \circ I' \colon X \to X$ the \textbf{canonical form of
$\sim$ determined by $I$ and $s'$}.
\hfill $\square$
\end{definition}

\begin{proposition}
% Lean: CanonicalForms.canonicalFormOfCompleteInvariant_isCanonicalForm (CanonicalForms/CanonicalForm.lean)
In the setting of Definition~4, the canonical form $s' \circ I'$ of $\sim$
determined by $I$ and $s'$ is a canonical form of $\sim$.
\hfill $\square$
\end{proposition}

\begin{proof}
By Proposition~3, $I' \colon X \to I(X)$ is a surjective complete invariant of
$\sim$. Apply Proposition~2 to $I'$, with $Y$ replaced by $I(X)$, and to
$C = s' \circ I'$. Since $s'$ is a section of $I'$ with $C = s' \circ I'$,
condition~(2) holds, and hence $s' \circ I'$ is a canonical form of $\sim$.
\end{proof}

\begin{lemma}\hspace{-0.7em}$^{\textrm{AC}}$\hspace{0.2em}
% Lean: CanonicalForms.surjective_has_section (CanonicalForms/Existence.lean)
...
% Lean: CanonicalForms.canonicalFormOfCompleteInvariant_exists (CanonicalForms/Existence.lean)
Let $X, Y$ be sets and let $I \colon X \to Y$ be a complete invariant of an
equivalence relation $\sim$ on $X$. Let $I' \colon X \to I(X)$ be the unique
map satisfying $I = i \circ I'$ with respect to the inclusion
$i \colon I(X) \to Y$. Then there exists a section $s' \colon I(X) \to X$ of
$I'$, and hence there exists a canonical form of $\sim$ determined by $I$ and
$s'$.
\hfill $\square$
\end{proposition}

\begin{proof}
By Proposition~3, $I'$ is surjective, so by Lemma~2 there exists a map
$s' \colon I(X) \to X$ satisfying $I' \circ s' = \mathrm{id}_{I(X)}$, that
is, a section of $I'$. By Proposition~4, the canonical form $s' \circ I'$ of
$\sim$ determined by $I$ and $s'$ is a canonical form of $\sim$.
\end{proof}
