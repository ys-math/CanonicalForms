...
- `canonicalFormOfCompleteInvariant_exists`（命題5）：主張を著者が与えた主張に置き換えました. 新しい主張は写像 `I : α → β` を取り, `s'` が `corestriction I` のセクションであることを述べます. 証明はAIが新しい主張に合わせて修正しました. また, AIが `docs/theory_en.tex` と `docs/theory_ja.tex` の命題5に `I` が完全な不変量であるという仮定を追加し, 両方のPDFを再生成しました.
